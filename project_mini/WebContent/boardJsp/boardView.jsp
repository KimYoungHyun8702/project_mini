    ...
    -->
		<c:choose>
			<c:when test="${requestScope.boardBean.previousPageGroup}">
				...
				<a
					href="${initParam.rootPath }/SelectController?page=${requestScope.boardBean.beginPage-1}">◀</a>
			</c:when>
			<c:otherwise>
         ◀
         </c:otherwise>
    ...
    -->
		<c:choose>
			<c:when test="${requestScope.boardBean.nextPageGroup }">
				<a
					href="${initParam.rootPath }/SelectController?page=${requestScope.boardBean.endPage+1}">▶</a>
			</c:when>
			<c:otherwise>
         ▶
         </c:otherwise>
		...
		<!-- 마지막 페이지로 이동 -->
		<a
			href="${initParam.rootPath }/SelectController?page=${requestScope.boardBean.totalPage}">마지막
			페이지</a>
</body>
</html>
